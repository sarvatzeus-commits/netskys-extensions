...
var _Sources = (() => {
  ...
  // src/Mangafire/MangafireUtils.ts
  ...
  var parseMangaDetails = async (source, $2, mangaId) => {
    const title = $2('h1[itemprop="name"]').text().trim() ?? "";
    const image = $2('img[itemprop="image"]').attr("src") ?? "";
    const description = $2('div[itemprop="description"]').text().trim() ?? "";
  ...
  var parseChapters = ($2, mangaId) => {
    const chapters = [];
    let sortingIndex = 0;
    for (const chapter of $2('a[href*="/read/"]').toArray()) {
  const href = $2(chapter).attr("href") ?? "";
  if (!href.includes("/chapter-")) continue;

  const id = href.split("/").pop()?.trim() ?? "";

  const chapMatch = id.match(/chapter-(\d+(\.\d+)?)/i);
  const chapNum = chapMatch ? Number(chapMatch[1]) : 0;

  chapters.push({
    id,
    name: `Chapter ${chapNum || id}`,
  ...
  var parseChapterDetails = async ($2, mangaId, chapterId) => {
    const pages = [];
    for (const image of $2('img').toArray()) {
  const img =
    $2(image).attr("data-src") ??
    $2(image).attr("data-original") ??
    $2(image).attr("src") ??
    "";
      if (!img) continue;
      let pageUrl = img;
...
pages.push(pageUrl);
  ...
  // src/Mangafire/Mangafire.ts
  var AS_DOMAIN = "https://mangafire.to";
  var AS_API_DOMAIN = "";
  var MangafireInfo = {
    version: "4.3.0",
    name: "Mangafire",
